## 1. Objetivo
Implementar en C/C++ el dibujo del fractal Burning Ship sobre el plano complejo usando CUDA.

El proyecto debe incluir una versión escalar de referencia y una versión CUDA equivalente, de forma que ambas produzcan la misma imagen final salvo por diferencias numéricas mínimas permitidas por la precisión de punto flotante.
...
- CUDA mediante kernels e intrínsecos del compilador
- Fuente Arial embebida con `arial_ttf.h` o, si no está disponible, una ruta de respaldo equivalente
...
- `render_fractal_cpu`: calcula la imagen completa como referencia correcta.
- `cuda_render_fractal`: calcula la misma imagen usando CUDA.
- El programa debe permitir alternar entre modo CPU y modo CUDA para comparar resultados.
...
## 5. Reglas de paralelización

- El kernel CUDA debe procesar bloques contiguos de píxeles.
- Debe existir un tramo seguro para los elementos restantes cuando el total no complete el bloque.
- La lógica del fractal debe mantenerse equivalente en ambas versiones.
- No se permite usar MPI ni distribución por procesos.
- No se permite delegar el cómputo principal a hilos de CPU; el foco del proyecto es CUDA.
...
| # | Criterio | Pts |
| :--- | :--- | :--- |
| 1 | `render_fractal_cpu` correcto y claro | 3 |
| 2 | `cuda_render_fractal` equivalente al escalar | 5 |
| 3 | Uso real de CUDA con bloques vectoriales y manejo de remanentes | 4 |
...
- Debe ser fácil comparar la versión CPU con la versión CUDA.
...
- una implementación CUDA funcional,
