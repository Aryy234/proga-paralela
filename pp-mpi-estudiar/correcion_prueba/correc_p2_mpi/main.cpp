...
void setup_iu()
{
    ...
    if (!texture.resize({WIDTH, HEIGHT})) {
        fmt::println("Error al crear la textura");
        running = 0;
    ...
    std::string options = "Ayuda: Flecha arriba/abajo cambia iteraciones | Esc cierra";
    ...
    int histograma_global[PALETTE_SIZE] = {0};
    ...
    while (window.isOpen())
    {
        ...
        burning_mpi(x_min, y_min, x_max, y_max, WIDTH, HEIGHT,
                    row_start, row_end, pixel_buffer, histograma_local);
        dibujar_texto(rank);
        ...
        std::string primera_fila_histograma = "";
        std::string segunda_fila_histograma = "";
        for (int bin = 0; bin < PALETTE_SIZE; bin++) {
            if (bin < PALETTE_SIZE / 2) {
                primera_fila_histograma += fmt::format("{}:{} ", bin, histograma_global[bin]);
            } else {
                segunda_fila_histograma += fmt::format("{}:{} ", bin, histograma_global[bin]);
            }
        }

        std::string mensaje = fmt::format(
            "Burning Ship MPI | RANKs: {} | Iteraciones: {} | FPS: {}\n"
            "Dominio: [{:.2f}, {:.2f}] x [{:.2f}, {:.2f}]\n"
            "Histograma: {}\n"
            "            {}",
            nprocs, max_iteraciones, fps,
            x_min, x_max, y_min, y_max,
            primera_fila_histograma, segunda_fila_histograma);
...
int main(int argc, char** argv)
{
    ...
    if (rank == 0) {
        ...
        while (true) {
            std::array<int, 2> estado;
            std::array<double, 4> plano;
