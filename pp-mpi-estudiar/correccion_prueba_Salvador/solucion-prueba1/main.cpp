...
void dibujar_histograma(int* hist) {
    std::string txt = "";
    for(int i=0;i<16;i++) {
        txt = txt + ", " + std::to_string(hist[i]);
    }
    auto texto = fmt::format("Histogram: [{}]", txt);
...
void setup_ui( ) {
    ...
    while (window.isOpen())
    {
        ...
        std::vector<int> global_hist(16*nprocs);
        ...
        //consolidar el historgrama global
        std::vector<int> final_hist(16);
        
        for(int i=0;i<16;i++) {
            final_hist[i] = 0;
            for(int j=0;j<nprocs;j++) {
                final_hist[i] += global_hist[j*16+i];
            }
        }

        auto hist_str = fmt::format("final_hist={}\n", final_hist);

        textOptions.setString(hist_str);
